...
#         """ Create and return a compiled LSTM model for KerasRegressor """
#         model = Sequential([
#             Input(shape=(60, 1)),  # Explicit Input layer
...
class GridSearchLSTM:
    @staticmethod
    def create_model(lstm_units=50, dropout_rate=0.2):
        """ Create and return a compiled LSTM model for GridSearch """
        model = Sequential([
            Input(shape=(60, 1)),  # Explicit Input layer
            LSTM(units=lstm_units, return_sequences=True),
            Dropout(dropout_rate),
    ...
    def grid_search_lstm(data):
        # Prepare the data
        tsla_data = data
        X = tsla_data.values.reshape(-1, 1)  # Reshape to a 2D array
        y = tsla_data.values

        # Define the KerasRegressor wrapper around the LSTM model
        model = KerasRegressor(
            build_fn=GridSearchLSTM.create_model, 
            verbose=0
        )
        ...
        best_model.model_.save('best_lstm_model.h5')  # Save the best model's underlying keras model

        return grid_result.best_params_

    @staticmethod
    def load_best_model():
        """ Load the best LSTM model from file. """
